(async function () {
    ...
    function createMenuChildren(id) {
        return `
            <div id="${id}" class="${id}--hide">
                <ul class="sidebar-dropdown__sale-list">
                    <li class="sidebar-dropdown__sale-item">
                        <a href="#" class="sidebar-dropdown__sale-link">
                            Gerador de Link
                        </a>
                    </li>
                    <li class="sidebar-dropdown__sale-item">
    ...
    function createMenuItem(title, index, hasChildren, idMenuItem) {
        const childrenId = `dropdown-${idMenuItem}`

        const template = `
            <li id="${idMenuItem}" class="sidebar__menu-item">
                <span class="sidebar__menu-link">
                    <div class="sidebar__menu-flex">
                        ...
                        ${hasChildren ? `
                            <svg id="icon${index + 1}" class="sidebar__menu-drop-icon" width="12" height="6" viewBox="0 0 12 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            ${hasChildren ? createMenuChildren(childrenId) : ''}
        `
    ...
    async function renderMenu() {
        ...
        const menuListElement = document.getElementById(MENU_LIST_ID);
        // Clear menu items
        menuListElement.innerHTML = '';
        
        const menuItems = await fetchMenuItems();

        if (menuItems != null && menuItems.menuTree.length > 0) {
            menuItems.menuTree.forEach((item, index) => {
                const menuItemId = `menu-item-${item.name.replace(' ', '').trim()}`
                const template = createMenuItem(item.name, index, item.hasChildren, menuItemId);
    ...
    function renderDropdown() {
        ...
        let dropDownIcon1 = document.getElementById('icon2')
        
        console.log(dropDownIcon1)
            
             dropdownToSale.addEventListener('click', function() {
                 if(dropDownMenu1.classList.contains("dropdown-menu-item-Pravender--hide")) {
                     // esconder outros menus abertos
                     dropDownMenu2.classList.add("dropdown-menu-item-Financeiro--hide")
                     dropDownMenu2.classList.remove("dropdown-menu-item-Financeiro--visible")
                     dropDownIcon2.classList.remove("iconRotate")
                    
                    dropDownMenu3.classList.add("dropdown-menu-item-Configurações--hide")
                    dropDownMenu3.classList.remove("dropdown-menu-item-Configurações--visible")
                    dropDownIcon3.classList.remove('iconRotate') 
                    
        
                    dropDownMenu4.classList.add("dropdown-menu-item-Ferramentas--hide")
                    dropDownMenu4.classList.remove("dropdown-menu-item-Ferramentas--visible")
                    dropDownIcon4.classList.remove('iconRotate');
                    
        
                    // abrir menu desejado
                    dropDownMenu1.classList.add("dropdown-menu-item-Pravender--visible")
                    dropDownMenu1.classList.remove("dropdown-menu-item-Pravender--hide")
                    dropDownIcon1.classList.add("iconRotate")
                } else {
                    dropDownMenu1.classList.add("dropdown-menu-item-Pravender--hide")
                    dropDownMenu1.classList.remove("dropdown-menu-item-Pravender--visible")
        ...
        let dropDownIcon2 = document.getElementById('icon4')
        
            dropdownToFinance.addEventListener('click', function() {
                if(dropDownMenu2.classList.contains("dropdown-menu-item-Financeiro--hide")) {
                    // esconder outros menus abertos
                    dropDownMenu1.classList.add("dropdown-menu-item-Pravender--hide")
                    dropDownMenu1.classList.remove("dropdown-menu-item-Pravender--visible")
                    dropDownIcon1.classList.remove("iconRotate")
                    ...
                    // abrir menu desejado
                    dropDownMenu2.classList.add("dropdown-menu-item-Financeiro--visible")
                    dropDownMenu2.classList.remove("dropdown-menu-item-Financeiro--hide")
        ...
        let dropDownIcon3 = document.getElementById('icon5');
            
            dropdownToConfig.addEventListener('click', function() {
                if(dropDownMenu3.classList.contains("dropdown-menu-item-Configurações--hide")) {
                   // esconder outros menus abertos
                   dropDownMenu1.classList.add("dropdown-menu-item-Pravender--hide")
                   dropDownMenu1.classList.remove("dropdown-menu-item-Pravender--visible")
                   dropDownIcon1.classList.remove("iconRotate")
                   ...
                   // abrir menu desejado
                   dropDownMenu3.classList.add("dropdown-menu-item-Configurações--visible")
                   dropDownMenu3.classList.remove("dropdown-menu-item-Configurações--hide")
         ...
         let dropDownIcon4 = document.getElementById('icon6')
            
             dropdownToTool.addEventListener('click', function() {
                 if(dropDownMenu4.classList.contains("dropdown-menu-item-Ferramentas--hide")) {
                     // esconder outros menus abertos
                     dropDownMenu1.classList.add("dropdown-menu-item-Pravender--hide")
                     dropDownMenu1.classList.remove("dropdown-menu-item-Pravender--visible")
                     dropDownIcon1.classList.remove("iconRotate")
                     ...
                     // abrir menu desejado
                     dropDownMenu4.classList.add("dropdown-menu-item-Ferramentas--visible")
                     dropDownMenu4.classList.remove("dropdown-menu-item-Ferramentas--hide")
